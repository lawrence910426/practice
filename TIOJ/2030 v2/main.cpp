...
public:
    shared_ptr<Node> l ,r;
    int val ,sum;
    Node(auto L ,auto R ,int V ,int S):l(L) ,r(R) ,val(V) ,sum(S) {}
    void Pull_Up() {
        this->sum = this->val;
...
class Persistent {
    shared_ptr<Node> root; int N;
    shared_ptr<Node> Build(int l ,int r) {
        if(l == r - 1) return make_shared<Node>(nullptr ,nullptr ,0 ,0);
        else return make_shared<Node>(Build(l ,(l + r) / 2) ,Build((l + r) / 2 ,r) ,0 ,0);
    }
    shared_ptr<Node> Modify(shared_ptr<Node> self ,int l ,int r ,int pos ,int diff) {
        shared_ptr<Node> ret = make_shared<Node>(*self);
        if(l == r - 1) ret->val += diff;
        else {
            int mid = (l + r) / 2;
            if(pos < mid) ret->l = Modify(self->l ,l ,mid ,pos ,diff);
            else ret->r = Modify(self->r ,mid ,r ,pos ,diff);
        }
        ret->Pull_Up();
        return ret;
    }
    int Query(shared_ptr<Node> self ,int l ,int r ,int ql ,int qr) {
        if(qr <= l || r <= ql) return 0;
        if(ql <= l && r <= qr) return self->sum;
        else return Query(self->l ,l ,(l + r) / 2 ,ql ,qr) + Query(self->r ,(l + r) / 2 ,r ,ql ,qr);
    }
public:
    void Modify(int pos ,int diff) { this->root = Modify(root ,0 ,N ,pos ,diff); }
    int Query(int l ,int r) { return Query(root ,0 ,N ,l ,r); }
    Persistent(int siz):N(siz) { this->root = Build(0 ,N); }
    Persistent(Persistent* p) { this->root = p->root; this->N = p->N; }
    Persistent() {}
};

class Seg {
    int N ,leaf;
    Persistent* data[MAXN * 3];
public:
    ~Seg() { for(int i = 1;i < leaf * 2;i++) delete data[i]; }
    Seg() {}
    inline Seg(int siz):N(siz) {
        for(leaf = 1;leaf < N + 2;leaf <<= 1) ;
        data[1] = new Persistent(N + 1);
        for(int i = 2;i < leaf * 2;i++) data[i] = new Persistent(data[i - 1]);
    }
...
/* Why persistent data structure makes MLE. I just dont give a fuck. */
